...
- (void)setUp
{
	...
	_sessionID = [auth sessionIDFromLoginWithUsername:SANDBOXED_USERNAME password:SANDBOXED_PASSWORD].body;
...
- (void)testFetchAncestors
{
	...
	FSAuth *auth = [[FSAuth alloc] initWithDeveloperKey:PRODUCTION_DEV_KEY sandboxed:NO];
	NSString *sessionID = [auth sessionIDFromLoginWithUsername:PRODUCTION_USERNAME password:PRODUCTION_PASSWORD].body;
	FSPerson *person = [FSPerson currentUserWithSessionID:sessionID];
	response = [person fetch];
	STAssertTrue(response.success, nil);
	
	response = [person fetchAncestors:3];
	STAssertTrue(response.success, nil);


	NSUInteger ancestorCount = 0;
	NSMutableArray *queue = [NSMutableArray arrayWithObject:person];
	while (queue.count > 0) {
		ancestorCount++;
	...
	STAssertTrue(ancestorCount == 31, nil);
}
