...
- (MTPocketResponse *)save
{
	...
	for (FSOrdinance *ordinance in _ordinances) {
		...
		if (ordinance.type)			[ordinanceInfo setObject:ordinance.type forKey:@"type"];
		if (ordinance.date)			[ordinanceInfo setObject:@{ @"original" : ordinance.date }	forKey:@"date"];
		if (ordinance.templeCode)	[ordinanceInfo setObject:ordinance.templeCode forKey:@"temple"];

		if (ordinance.type && ordinance.date) {
			[events addObject: @{ @"value" : ordinanceInfo } ];
		}
	}
...
- (void)addUnofficialOrdinanceWithType:(FSOrdinanceType)type date:(NSDate *)date templeCode:(NSString *)templeCode
{
	...
	FSOrdinance *ordinance = [FSOrdinance ordinanceWithType:type identifier:nil];
	ordinance.date			= date;
	ordinance.templeCode	= templeCode;
